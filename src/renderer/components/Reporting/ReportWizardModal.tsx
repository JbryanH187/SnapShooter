...
import { FileText, FileType, Download, Check, Settings, Palette, Upload, X, RefreshCw, Save } from 'lucide-react';
...
export const ReportWizardModal: React.FC<ReportWizardModalProps> = ({ isOpen, onClose, captures, authorName, initialDraft }) => {
    ...
    const handleSaveDraft = async () => {
        ...
        try {
            ...
            if ((window as any).electron?.saveReportDraft) {
                await (window as any).electron.saveReportDraft(draft);
                setDraftId(draft.id);
    ...
    const handleExport = async () => {
        setIsExporting(true);
        try {
            // 1. Generate blob for history (and saving)
            ...
            if (blob) {
                ...
                console.log('[ReportWizard] Triggering download:', fileName, 'Size:', blob.size);
                ...
                // Save file to userData/reports/ via IPC and get the path back
                let savedFilePath: string | null = null;
                try {
                    const arrayBuffer = await blob.arrayBuffer();
                    if ((window as any).electron?.saveReportFile) {
                        savedFilePath = await (window as any).electron.saveReportFile(fileName, arrayBuffer);
                    }
                } catch (err) {
                    console.error("Failed to save report file:", err);
                }
                ...
                if ((window as any).electron?.saveReportToHistory) {
                    await (window as any).electron.saveReportToHistory({
